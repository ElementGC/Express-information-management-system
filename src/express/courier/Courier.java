    ...
    public Courier(){
        ...
        bt1.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (enquiry.startsWith("1")) {
                    try {
                        //从输入的文本框里获取输入的数据，然后做对比
                        //'"+account+"'这里这个表示的是变量account
                        String sql = "update expressinformation set expresslocation = '" + loc + "',expressstatus = '" + state + "' where goodnum = '"+enquiry+"'";
                    ...
                    try {
                        //从输入的文本框里获取输入的数据，然后做对比
                        //'"+account+"'这里这个表示的是变量account
                        String sql = "update expressinformation set expresslocation = '" + loc + "',expressstatus = '" + state + "' where receiver = '"+enquiry+"'";
                        GroupLayoutTest.statement = GroupLayoutTest.conn.createStatement();
                        int res = GroupLayoutTest.statement.executeUpdate(sql);
                        if (res == 1){
                            JOptionPane.showMessageDialog(null, "录入成功！",
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (enquiry.startsWith("1")) {
                    try {
                        //从输入的文本框里获取输入的数据，然后做对比
                        //'"+account+"'这里这个表示的是变量account
                        String sql = "select * from expressinformation where goodnum = '"+enquiry+"'";
                        GroupLayoutTest.statement = GroupLayoutTest.conn.createStatement();
                        ResultSet res = GroupLayoutTest.statement.executeQuery(sql);
                        String s = null;
                        while (res.next()){
                            s +="编号为" + res.getString("goodnum") + "的快递";
                    ...
                    try {
                        //从输入的文本框里获取输入的数据，然后做对比
                        //'"+account+"'这里这个表示的是变量account
                        String sql = "select * from expressinformation where receiver = '"+enquiry+"'";
                        GroupLayoutTest.statement = GroupLayoutTest.conn.createStatement();
                        ResultSet res = GroupLayoutTest.statement.executeQuery(sql);
                        String s = null;
                        while (res.next()){
        ...
        hGroup.addGroup(layout.createParallelGroup().addComponent(back).addComponent(label1).
                ...
                .addComponent(bt2));
        hGroup.addGap(5);
        hGroup.addGroup(layout.createParallelGroup().addComponent(tf1).addComponent(tf3));
        hGroup.addGap(10);
